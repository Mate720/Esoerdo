<template>
  <div ref="rootRef" class="relative w-full h-full box-border" :style="{
    '--segments-x': segments,
    '--segments-y': segments,
    ...
    <main ref="mainRef"
      ...
      <div class="w-full h-full grid place-items-center contain-layout contain-paint contain-size" :style="{
        perspective: 'calc(var(--radius) * 2)',
        perspectiveOrigin: '50% 50%'
      }">
        <div ref="sphereRef" class="will-change-transform"
          style="transform-style: preserve-3d; transform: translateZ(calc(var(--radius) * -1))">
          <div v-for="(item, i) in items" :key="`${item.x},${item.y},${i}`"
            class="absolute -top-[999px] -bottom-[999px] -left-[999px] -right-[999px] m-auto transition-transform duration-300"
            :data-src="item.src" :data-offset-x="item.x" :data-offset-y="item.y" :data-size-x="item.sizeX"
            :data-size-y="item.sizeY" :style="{
              '--offset-x': item.x,
              '--offset-y': item.y,
            ...
            <div
              ...
              role="button" tabindex="0" :aria-label="item.alt || 'Open image'" @click="onTileClick"
              @pointerup="onTilePointerUp" @touchend="onTileTouchEnd" :style="{
              ...
              <img :src="item.src" draggable="false" :alt="item.alt"
                class="w-full h-full object-cover pointer-events-none" :style="{
                  backfaceVisibility: 'hidden',
      ...
      <div class="absolute inset-0 m-auto z-[3] pointer-events-none" :style="{
      ...
      <div class="absolute left-0 right-0 h-[120px] z-[5] pointer-events-none top-0 rotate-180" :style="{
        background: 'linear-gradient(to bottom, transparent, var(--overlay-blur-color, #060010))'
      }" />
      <div class="absolute left-0 right-0 h-[120px] z-[5] pointer-events-none bottom-0" :style="{
        background: 'linear-gradient(to bottom, transparent, var(--overlay-blur-color, #060010))'
      }" />

      <div ref="viewerRef" class="absolute inset-0 z-20 pointer-events-none flex items-center justify-center"
        ...
        <div ref="frameRef" class="h-full aspect-square flex max-[1/1]:h-auto max-[1/1]:w-full"
