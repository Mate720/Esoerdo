...
const { questions, next, correct, actual, checkAnswer, onSelect, reset, resultMessage, quit, confirmDialogAnswer, confirmDialogVisible } = useAnimalQuestions();
</script>
            ...
            <template #container="{ closeCallback }">
                <div class="flex flex-col px-8 py-8 gap-6 rounded-2xl"
                    ...
                    <div class="flex gap-4">
                        <Button label="Cancel" @click="() => { confirmDialogAnswer(false); closeCallback(); }"
                            variant="text"
                            class="!p-4 w-full !text-primary-50 !border !border-white/30 hover:!bg-white/10" />
                        <Button label="Confirm" @click="() => { confirmDialogAnswer(true); closeCallback(); }"
                            variant="text"
                            class="!p-4 w-full !text-primary-50 !border !border-white/30 hover:!bg-white/10" />
                    </div>
                </div>
