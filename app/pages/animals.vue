...
const checkAnswer = () => {
  ...
  if (selectedIndex.value === -1) {
    ...
    selectedIndex.value === AnimalQuestions[actual.value]?.correctIndex
  ) {
    correct.value++;
    toast.add({
      severity: "success",
...
<template>
  ...
  <section v-if="actual < 10">
    ...
    <div>
      ...
      <button class="bg-accent p-2 text-xl rounded-md cursor-pointer" type="button" @click="checkAnswer()">
        Check answer
      </button>
    </div>
  </section>
  ...
  <section v-else>
    ...
    <div class="flex justify-center">
      ...
      <button class="bg-accent p-2 text-xl rounded-md cursor-pointer" type="button" @click="reset()">
        Restart
      </button>
    </div>
  </section>
